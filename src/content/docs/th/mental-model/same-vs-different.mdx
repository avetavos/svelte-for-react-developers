...
- **ไม่มี virtual DOM** ไม่มี reconciler ไม่มี fiber ไม่มี "batched re-render" Update คือ direct DOM write แบบ surgical คุณไม่สามารถเรียก `forceUpdate()` หรือ trigger full re-render ได้ — ไม่ใช่ concept ที่มีอยู่
...
- **Children คือ snippet ไม่ใช่ value** `{@render children()}` เรียก snippet function คุณไม่สามารถ spread children, map ทับ หรือส่งต่อเป็น arbitrary value แบบที่ React ให้ treat `children` เป็น node tree
